...
and compatibility strategy. `AGENTS.md` explicitly lists nine ADRs as required before
implementation begins.

This document identifies all twenty-two ADRs needed to fully cover the decisions already
encoded in the process documentation. They are organized into four tiers by what they
unblock, with a meta-ADR (0001) that must precede all others because it defines the
format every subsequent ADR must follow.

---

## Tier 0 — Meta (must be written first)
...
This resolves OQ-03 and OQ-29 and is the template every other ADR on this list must
follow.
...
never reverse-engineers intent, and the spec-first discipline reduces agent
hallucination. Covers the six-phase planning lifecycle, gate requirements between
phases, and why formal spec is required before implementation begins. This governs every
planning decision downstream.
...
their preconditions, `Blocked` as a returnable state from any active state, and
Linear as the system of record. Resolves OQ-24.
...
engineering perspective) and intake (Drafter, product/stakeholder perspective). Covers
the dual meaning of each pass by role, why "Refine" is the canonical name for the third
pass, what decision-completeness means for an implementer, and the separation between
ideation (CTR intake) and formal specification (CTR planning). Covers the Draft Design
Prompt format as the mandatory handoff artifact from the Drafter to the Architect.
...
implementation conventions and quality gates that every PR must satisfy.
...
workflow, platform boundaries), the eight blocker conditions, and the review service
level targets.
...
Required before the system operates autonomously without human supervision. These eight
ADRs cover the operational mechanics of orchestration, resilience, security, and
infrastructure.

### ADR-0011: Director Orchestration Model — Polling vs. Webhook

Decide between 5-minute polling (simpler, current default) and webhook-driven dispatch
(lower latency, higher infrastructure complexity). Covers the tradeoff analysis,
selected approach, fallback behavior during webhook outage if webhooks are chosen, and
polling interval configuration. This is ADR Backlog Item #1 in `AGENTS.md`. Resolves
OQ-08.
...
in-progress session, and lock orphan handling. This is ADR Backlog Item #2 in
`AGENTS.md`.

### ADR-0013: Concurrency, Ticket Locking, and Branch Namespace

Define the ticket lock and branch namespace lock mechanisms that prevent agent
collisions on concurrent tasks. Covers lock acquisition rules (at `Selected` → `In
Progress`), release rules (at `Done`, cancellation, or Director recovery), lock
reconciliation procedure (requires human confirmation), and the policy for multiple
Engineers working simultaneously on independent tasks. Resolves OQ-05 and OQ-24.
...
upstream tasks reach `Done`. Covers the trigger (Director re-invokes Coordinator on
newly `Done` tasks with `Backlog` dependents), the dependency resolution algorithm, and
the guarantees required before promotion (all upstream tasks `Done`, no active locks on
shared subsystems).
...
session lifecycle, and what happens when the execution substrate is unavailable.
...
creation at task start (at `Selected` → `In Progress`), retention through PR merge,
cleanup after merge, recreation on task rejection requiring restart, and isolation
guarantees between concurrent worktrees. This decision has cross-cutting impact on
every Engineer invocation.
...
Define what the Constitution (`.specify/memory/constitution.md`) is, its required
sections, who ratifies it, when it is evaluated (Constitution Check during planning),
and what happens when a plan violates it. Distinguish between the structural decision
(captured here) and the actual engineering principles (content of the Constitution
itself, which is a separate document). Resolves OQ-31.
...
(tests, lint, type checks, build), the policy for divergence between local pass and CI
failure, and coverage threshold requirements. Resolves OQ-07 and OQ-13.
...
Covers how agents must populate the template and what missing sections constitute a
...
Write in this sequence. Each ADR depends on or references those before it.
...
**Minimum set (unblocks implementation):** ADR-0001 through ADR-0010 (Tiers 0–2).
These ten cover the decisions directly required before an Engineer can begin work under
human supervision. `AGENTS.md` identifies nine pre-implementation ADRs; this adds
ADR-0001 (meta-ADR) as a necessary prerequisite, yielding ten.

**Full set (unblocks full autonomy):** ADR-0001 through ADR-0022 (all tiers). Required
...
1. Write **ADR-0001** first to establish the format and criteria.
