# Merge Proposal: Software Review Process

## Merger Rationale

Three agents (Claude, Codex, Gemini) produced near-identical review process
proposals. The substantive content is unanimous. This merge canonicalizes the
three into a single document by resolving the two trivial formatting
divergences:

- **Title**: Drop "Proposal" from the title. All three agents framed this as a
  process definition, not a proposal under debate. The Gemini agent signaled
  this by naming its file `review-process.md`. The merged document is the
  process.
- **Heading capitalization**: Use sentence case throughout (Claude/Gemini
  style). Sentence case is the consistent convention in this repository and
  produces cleaner markdown.

No content was added, removed, or arbitrated. There was nothing to arbitrate.
