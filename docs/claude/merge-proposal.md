...
Adoption of this agent requires an ADR because it introduces the first role in the
system that is invoked by a human directly rather than dispatched by the Director.
...
This assumption is routinely false. In practice, stakeholders arrive with half-formed
ideas, ambiguous goals, and undiscovered constraints.

The documentation already signals this gap in multiple places:
...
- `docs/open-questions.md` explicitly flags this in OQ-06: "Should a future Triage
  agent create `Draft` issues from raw requests?"

The consequence is that the Architect absorbs two fundamentally different modes of
work: exploratory stakeholder interviewing (conversational, undirected) and formal
planning artifact production (disciplined, gate-governed). Splitting them produces a
cleaner handoff and protects planning capacity.
...
4. Produce a CTR design prompt that is concise but planning-ready (see format below).
5. Present the completed CTR block to the human and obtain confirmation before writing
   to Linear.
...
- No deep repository or code exploration (see note on lightweight context below)
...
**product and stakeholder perspective**. The naming of the third pass is deliberately
kept as **Refine** to remain consistent with the Architect's CTR vocabulary, but the
content differs:
...
| **R — Refine** | Tighten until decision-complete for implementer | Clarify assumptions, surface unresolved questions, identify risks, note research areas for the Architect or Explorer, document non-goals, state the classification rationale |

The output of this CTR pass is a **design prompt** — not a spec, not a plan — but a
well-structured statement of intent that gives the Architect a solid foundation without
forcing them to re-interview the stakeholder.

The third pass ("Refine") deliberately bridges what the Claude proposal called
"Requirements" (must-haves, constraints, scope boundaries) and what the Gemini design
called "Result" (success criteria, definition of done). Both are required. Refine is the
pass that captures all of it: constraints, must-haves, explicit non-goals, measurable
success criteria, and open questions that remain for planning.

---

## Conversation Protocol

The Drafter conducts the intake conversation in three passes with a hard cap of
**nine questions across all passes**. Quality of questions matters more than quantity.
...
- What constraints exist (compliance, performance, compatibility, dependencies,
  budget)?
...
stakeholder, and waits for explicit confirmation before creating the Linear issue.
The stakeholder must either approve the prompt as written or request corrections. The
Drafter iterates until the stakeholder is satisfied or acknowledges remaining open
questions.
...
- The Drafter may perform lightweight context gathering (e.g., checking existing agent
  names or classification taxonomy) but must not perform deep code exploration. That
  boundary is enforced by tool access, not agent judgment.
...
The argument for a new `Idea` or `Drafting` state (raised in two of the source
proposals) is weaker than the argument against it: adding a state requires updates to
`docs/tracking-process.md`, `docs/agentic-team.md`, the Director's dispatch logic, and
the Linear project configuration — significant coordination cost for a benefit (tracking
in-flight conversations) that can be achieved more cheaply by saving a local
`design-prompt.md` until the conversation is complete.

### Director Dispatch

The Director does **not** dispatch the Drafter. The Drafter is the system entry point,
invoked directly by a human. Once the `Draft` issue exists, the Director's existing
polling detects it and dispatches the Architect per the current T-1 gate. No change to
Director behavior is required.

### New Speckit Command: `/speckit.draft`

Add `/speckit.draft` to the speckit command suite. This is the canonical invocation
point for the Drafter, consistent with the existing `/speckit.*` command vocabulary.

```
/speckit.draft  →  Drafter  →  Draft Linear issue
```

The command definition should be created at:
...
**claude-haiku-4-5** (Claude), **gemini-flash** (Gemini), or equivalent fast model.

The primary task is structured conversational elicitation, not deep analysis. Response
latency matters for interactive use. However, where the conversation reveals unexpected
technical complexity, the agent should escalate to a higher-reasoning model (e.g.,
`claude-sonnet-4-6`) for that session rather than attempting to resolve the complexity
with a lower-capability model.
...
- No existing `Draft` issue for the same request (agent checks for duplicates before
  creating; surfaces existing issue if found)

### Exit Conditions

- CTR design prompt confirmed by stakeholder
- Linear issue created in `Draft` state with design prompt and intake metadata
- Execution log entry written (see format below)
...
Upon completion, the Drafter appends to the Linear issue (consistent with the system-wide
...
The current system invokes agents by Linear state, not direct call. This agent acts
...
exception in an ADR. Keep the rest of the lifecycle unchanged.

### Noisy or Low-Value Linear Tickets

If every rough idea becomes a `Draft` issue, Linear fills with speculative requests
that are not ready for planning.

Mitigation: require a minimum quality bar (defined objective, named requestor,
at least one success criterion) before ticket creation. The verification gate enforces
this — if the CTR block is too thin, the stakeholder is unlikely to approve it.
...
Cons: adds lifecycle complexity; requires updates to tracking-process.md,
agentic-team.md, Director dispatch logic, and Linear project configuration. The benefit
(tracking in-flight conversations) is achievable more cheaply with a local file.
...
| ID | Question | Source |
|----|----------|--------|
| OQ-33 | Who designates the stakeholder when a request has no named owner (e.g., monitoring alert, automated process)? | Claude |
| OQ-34 | Should the design prompt be stored only in the Linear issue description, or also committed to `specs/<###>/design-prompt.md`? The issue number is unknown until after creation. | Claude |
| OQ-35 | Should the Drafter validate that no `Draft` issue already exists before creating, or is duplication management the Architect's responsibility? | Claude |
| OQ-36 | What is the minimum quality bar (defined objective, named requestor, success criterion) that must pass before ticket creation is allowed? Who defines it? | Codex |
| OQ-37 | When the conversation transcript cannot be linked (e.g., CLI session), what is the acceptable traceability substitute — a summary paragraph, turn count, or nothing? | Gemini design |
| OQ-38 | Narrow Feature Draft Agent vs. broader future Triage Agent: at what point should the Drafter's scope expand to cover bug reports, chores, and operational requests? | Codex |
...
This proposal merges and supersedes:
...
- `docs/gemini/feature-draft-agent-design.md`
