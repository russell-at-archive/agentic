# Merged Proposal: ADR Backlog for Agentic Delivery System

**Date**: 2026-03-17
**Status**: Draft
**Merged From**: 
- `docs/gemini/adr-proposal.md`
- `docs/claude/adr-proposal.md`
- `docs/codex/adr-proposal.md`

## 1. Executive Summary

This document consolidates three independent ADR proposals into a single, tiered roadmap. It identifies 17 Architecture Decision Records (ADRs) required to formalize the system's design, resolve outstanding open questions (OQs), and unblock implementation. The backlog is organized into four tiers based on their impact on the delivery lifecycle and implementation sequencing.

---

## 2. Tier 1: Foundational Governance (Critical Path)
Nothing proceeds without these five. They define the foundational decisions that all agent dispatch logic, planning gates, and process documents depend on.

| ADR # | Title | Purpose | Resolves |
| --- | --- | --- | --- |
...
| **ADR-004** | **Nine-State Linear Machine** | Canonicalizes state names, the 8 gate transitions, and Linear as the system of record. | OQ-24 |
...
These ADRs must exist before any Engineer begins implementation work.
...
These decisions are needed before the system operates autonomously without human supervision.
...
## 5. Tier 4: Product & Operational Governance
Non-blocking but required for a complete, self-consistent architectural record.
...
## 6. Recommended Sequencing Note

1.  **Start with ADR-001**: This establishes the format for all subsequent records.
2.  **Ratify Tier 1**: This unblocks the Architect and Coordinator roles.
3.  **Ratify Tier 2**: This unblocks the Engineer and Technical Lead roles for a Phase 2 pilot.
4.  **Phase 3/4**: Tier 3 ADRs can be drafted during the Phase 2 pilot to prepare for full autonomy.
