...
1. `Workflow Isolation`
   - Launches each agent session inside a per-issue Docker container.
   - Builds or pulls the configured container image before first use.
   - Mounts the per-issue workspace directory into the container at a well-known path.
   - Injects required environment variables (tracker auth, agent credentials) into the container
     at runtime without persisting them to the image or workspace.
   - Enforces network, filesystem, and process isolation boundaries defined by the container
     runtime.
   - Terminates and removes the container when the agent session ends (success, failure, timeout,
     or cancellation).
...
7. `Agent Runner`
   ...
   - Launches the coding agent app-server client inside the isolation container.
...
- Docker daemon (or compatible OCI runtime) accessible to the orchestrator process.
- Container image containing the coding-agent executable and its runtime dependencies.
...
## 9.6 Workflow Isolation (Docker Container)

Each agent session runs inside a dedicated Docker container. The container provides the primary
isolation boundary between agent execution and the host system.

### 9.6.1 Container Configuration Schema

`isolation` front-matter object (top-level key in `WORKFLOW.md`):

- `image` (string, required)
  - OCI image reference used for every agent container (for example `ghcr.io/org/symphony-agent:1.0`).
  - Must include a tag or digest; bare image names without a tag are rejected at startup validation.
- `pull_policy` (string, optional)
  - Controls when the image is pulled before container creation.
  - Values: `always`, `if_not_present`, `never`.
  - Default: `if_not_present`.
- `workspace_mount` (string, optional)
  - Absolute path inside the container where the per-issue workspace directory is mounted.
  - Default: `/workspace`.
- `network` (string, optional)
  - Docker network mode passed to container creation.
  - Default: `none` (no network access inside the container).
  - Implementations that require the agent to reach external services (for example Linear) must
    document this explicitly and set an appropriate network mode.
- `env_passthrough` (list of strings, optional)
  - Names of host environment variables to inject into the container at runtime.
  - Variables are passed by value at container start; they are never written to the image or
    workspace filesystem.
  - Default: empty list.
- `extra_args` (list of strings, optional)
  - Additional arguments appended verbatim to the `docker run` invocation.
  - Use sparingly; prefer explicit schema fields for common options.
  - Default: empty list.

### 9.6.2 Container Lifecycle

Per-session lifecycle:

1. `Image preparation` — pull or verify the configured image according to `pull_policy` before
   launching the first container. Failure aborts the run attempt.
2. `Container creation` — create a new container for each run attempt with:
   - the per-issue workspace directory bind-mounted read-write at `isolation.workspace_mount`.
   - `isolation.env_passthrough` variables injected from the host environment.
   - the configured network mode.
   - the coding-agent command (`codex.command`) as the container entrypoint/command.
3. `Session execution` — the Agent Runner communicates with the coding agent over the container's
   stdio exactly as described in Section 10. The container replaces the local subprocess; the
   JSON-RPC protocol is identical.
4. `Teardown` — when the session ends (success, failure, timeout, or cancellation), stop and
   remove the container. Workspace contents on the host are preserved per normal workspace
   persistence rules (Section 9.1).

Container naming convention (recommended):

- `symphony-<sanitized_issue_identifier>-<attempt>` to make containers identifiable in
  `docker ps` output during operator debugging.

### 9.6.3 Safety Invariants

Invariant 4: Each run attempt uses a fresh container.

- Do not reuse containers across run attempts for the same issue or across issues.
- Container state (writable layers, processes) must not persist between attempts.

Invariant 5: Workspace mount is the only writable host path exposed to the container.

- No other host paths may be bind-mounted into the container unless explicitly configured via
  `isolation.extra_args` and documented in the implementation's trust posture statement.

Invariant 6: Credentials are injected at runtime, not baked into images.

- API tokens and secrets must be passed via `isolation.env_passthrough` or equivalent runtime
  mechanism.
- Images must not contain credentials in layers, environment directives, or embedded config files.

### 9.6.4 Startup Validation

Additional validation checks when isolation is configured:

- `isolation.image` is present and non-empty.
- `isolation.image` includes a tag or digest (bare name without tag is rejected).
- `isolation.pull_policy` is one of the supported values when present.
- `isolation.workspace_mount` is an absolute path when present.
- Docker daemon is reachable (implementations may defer this check to first dispatch rather than
  startup if a lightweight ping is not available).

Isolation config errors block new dispatches with the same semantics as other preflight validation
failures (Section 6.3).

### 9.6.5 Hook Execution Inside Containers

Workspace hooks (`after_create`, `before_run`, `after_run`, `before_remove`) run on the host
outside the container, with the workspace directory as `cwd`, consistent with Section 9.4.

Rationale: hooks perform host-side setup (for example repository clone, dependency bootstrap) that
must complete before the container is created. Running hooks inside the container would require a
separate container invocation per hook, complicating lifecycle ordering.

Implementations that require hook logic to run inside the container should do so via a custom
entrypoint script baked into the image rather than via Symphony's hook mechanism.
...
- Workflow isolation: each agent session runs in a dedicated Docker container (Section 9.6)
- Isolation config validated at startup: `isolation.image` required with tag/digest; `pull_policy`,
  `workspace_mount`, `network`, and `env_passthrough` validated when present
- Per-attempt container lifecycle: fresh container per attempt, stopped and removed on session end
- Workspace bind-mounted into container at `isolation.workspace_mount`; no other host paths exposed
- Credentials injected via `env_passthrough` at runtime; not baked into image
- Hooks execute on host outside container (Section 9.6.5)
- Coding-agent app-server subprocess client with JSON line protocol (via container stdio)
