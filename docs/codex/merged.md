...
This document defines a single task-tracking process for the repository. It
follows the Codex merge proposal as the base model and incorporates Claude's
gate rules and label taxonomy where they improve operational clarity.

The process is designed to be:

- simple enough to use consistently
- rigorous enough to preserve traceability and delivery quality
- explicit enough for both human developers and AI agents to collaborate safely

Planning artifacts define approved work. Task tracking records the live
...
4. Require explicit ownership, blocker handling, PR traceability, and
   completion evidence.
5. Require agents to leave resumable execution notes when they materially
   advance or block a task.

This keeps task tracking operationally strong without turning it into a second
planning system.
...
- Keep the live state model small and durable.
...
part of the required live execution workflow for approved tasks.
...
These gates apply to live execution:
...
Only tasks whose dependency tasks are `done` should enter `ready`.
...
5. apply the `status:in-progress` label
6. add the branch name to the issue
7. add a short execution note if there is relevant context for handoff
...
4. apply the `status:in-review` label
5. attach the validation evidence available so far
...
Apply the `status:done` label only after those conditions are satisfied.
...
This process keeps Codex's pragmatic execution model while incorporating
Claude's gate rules and label taxonomy where they add useful control. The
result is a single, coherent standard:

- planning artifacts define approved work
- issues carry live execution state
